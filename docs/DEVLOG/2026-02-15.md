...
### 5.4 Post-Integration Fixes (Frontend/Backend Contract Alignment)

Fixed response field mismatch in UI:

- `document.filename` → `document.original_filename` (matches Laravel `DocumentController::show()` payload)

Updated status badge mapping to match backend enum values:

- Handles: `UPLOADED`, `PROCESSING`, `PROCESSED`, `VALIDATED`, `FAILED`

Fixed form value serialization to match backend validation rules:

- `total_ttc` now sends `null` when cleared instead of `""` to satisfy `nullable|numeric`
- Added safer number parsing (comma → dot, `Number.isFinite`) to avoid NaN submissions
- `invoice_date` now sends `null` when cleared instead of empty string (keeps payload consistent)

**Result:** validation endpoint no longer fails on empty numeric/date edits; UI reflects backend fields correctly.

**Files updated:** `frontend/src/pages/DocumentValidation.jsx`
